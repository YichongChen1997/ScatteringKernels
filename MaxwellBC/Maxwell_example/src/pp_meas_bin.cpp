...
int main()
{
    ...
    int skipTimeStep = 70; // when to start measuring bins -> after steady state

    int tSkip = 5000;
    double deltaT = 2.0;
    double mi = 2.66389e-26; // mass of one fluid molecule

    double binWidth = 5;    // coarse
    double binWidth2 = 1;   // fine
    ...
    {
        ofstream binFile("maxwell_bins_coarse.txt", ios::out);
    ...
    {
        ofstream binFile("maxwell_bins_fine.txt", ios::out);
        ...
        for (i = 0; i < nBins2; i++)
        {
            ...
            binVol = binWidth2 * Lx * Ly * refVolume;
