...
int main()
{
    ...
    ofstream depthDis("depthDis.txt");
    // ************************ Penetration Depth Distribution ************************
    {
        vector<double> distributionPos(nzBins, 0.0);
        vector<double> distributionNeg(nzBins, 0.0);
        vector<double> binsPos(nzBins, 0.0);
        vector<double> binsNeg(nzBins, 0.0);

        for (i = 0; i < nzBins; i++)
        {
            binsPos[i] = depWidth * 0.5 + depWidth * i;
            binsNeg[i] = -binsPos[i];
        }
        ...
        for (i = 0; i < nzPts; i++)
        {
            if (zDepth[i] < rCut)
            {
                ...
                {
                    bo = floor(abs(zDepth[i]) / depWidth);
        ...
        vector<double> probabilityPos(nzBins, 0.0);
        vector<double> probabilityNeg(nzBins, 0.0);

        // scale wth area
        double areaUnderGraph = 0.0;

        for (i = 0; i < nzBins - 1; i++)
        {
        ...
        {
            depthDis << binsNeg[nzBins - i - 1] << " " << distributionNeg[nzBins - i - 1] << " " << probabilityNeg[nzBins - i - 1] << endl;
        }

        for (int i = 0; i < nzBins; i++)
        {
            depthDis << binsPos[i] << " " << distributionPos[i] << " " << probabilityPos[i] << endl;
        }
    }
    // ************************ Subfunction Ends ************************

    // ******************************************************************
    // Overall tangential velocity distributions depending on the penetration depth
    // ******************************************************************
    ofstream depthOverallDis_Tx("depthOverallDis_Tx.txt");

    vector<double> nDepthID = linspace(-rCut, rCut, 10);
    vector<vector<double> > DepthDisPos;
    vector<vector<double> > DepthDisNeg;
    vector<vector<double> > DepthProPos;
    vector<vector<double> > DepthProNeg;
    for (int m = 0; m < nDepthID.size(); m++)
    {
        ...
        DepthDisNeg.push_back(distributionNeg);
        DepthDisPos.push_back(distributionPos);
        DepthProNeg.push_back(probabilityNeg);
        DepthProPos.push_back(probabilityPos);

        for (int i = 0; i < nBins; i++)
        {
            depthOverallDis_Tx << binsNeg[nBins - i - 1] << " " << DepthDisNeg[m][nBins - i - 1] << " " << DepthProNeg[m][nBins - i - 1] << endl;
        }

        for (int i = 0; i < nBins; i++)
        {
            depthOverallDis_Tx << binsPos[i] << " " << DepthDisPos[m][i] << " " << DepthProPos[m][i] << endl;
        }
    }
    // ************************ Subfunction Ends ************************
    ...
    // ******************************************************************
    // Overall normal velocity distributions depending on the penetration depth
    // ******************************************************************
    ofstream depthOverallDis_N("depthOverallDis_N.txt");

    vector<vector<double> > DepthDis;
    vector<vector<double> > DepthPro;
    ...
    for (int m = 0; m < nDepthID.size(); m++)
    {
        ...
        DepthDis.push_back(distribution);
        DepthPro.push_back(probability);

        for (int i = 0; i < nBins; i++)
        {
            depthOverallDis_N << bins[i] << " " << DepthDis[m][i] << " " << DepthPro[m][i] << endl;
        }
