...
int main()
{
    ...
    double NorV = sqrt(kB * Tw / mGas);   // reference velocity

    ifstream data("dump_meas_gas.lammpstrj", ios::in);

    ofstream bottomVelocities("bottomVelocities.txt", ios::out);
    ...
    {
        bottomVelocities << vxStart[i] << " " << vyStart[i] << " " << vzStart[i] << " " << vxEnd[i] << " " << vyEnd[i] << " " << vzEnd[i] << endl;
        coBottom++;
    }
