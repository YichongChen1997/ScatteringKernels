...
int main()
{
    ...
    int atoms, typ, i, j, k, c, m, id, mol = 0, molId = 0, bID, aID, nCNT = 0, nWallCount = 0, nWallK = 0, nKWallCount = 0, co, co2;
    int nAtomsTot = 0;
    ...
    double atomForceSi = 1.0e-15;                       // Newtons (ensure the magnitude of velocity at decades of m/s)
    ...
    if (setPressure)
    {
        ...
        if (p2 < 50)
        {
            rho2 = 0.000009592344 * p2 * p2 * p2 * p2 + (-0.001207211400) * p2 * p2 * p2 + 0.022536131000 * p2 * p2 + 4.531223300000 * p2 + 0.021405916000;
        }
        else
        {
            rho2 = -0.000000399685 * p2 * p2 * p2 * p2 + 0.000213941490 * p2 * p2 * p2 + (-0.046716196000) * p2 * p2 + 5.698349000000 * p2 + (-0.360298820000);
        }
    }
    ...
    // create adsorbant
    int nAdsorbant = ceil(rho2 * volume / mi);

    cout << "number of adsorbed atoms to insert = " << nAdsorbant << endl;
    ...
    vector<double> zA(nAdsorbant);
    vector<double> xA(nAdsorbant);
    vector<double> yA(nAdsorbant);

    double rhoA = nAdsorbant / (Lx * Ly * Lz);
    double Nbox2 = rhoA * Lx * Lx * Lx;
    int nA = int(pow(Nbox2, 1.0 / 3));

    cout << "nA = " << nA << endl;

    nPx = int(((Lx - offsetX) * nA / Lx) + 1);
    nPy = int(((Ly - offsetY) * nA / Lx) + 1);
    nPz = int(((Lz - offsetZ) * nA / Lx) + 1);
    ...
    uCX = (Lx - offsetX) / double(nPx);
    uCY = (Ly - offsetY) / double(nPy);
    uCZ = (Lz - offsetZ) / double(nPz);
    ...
    for (j = 0; j < nPy; j++)
    {
        for (k = 0; k < nPz; k++)
        {
            for (i = 0; i < nPx; i++)
            {
                if (co2 < nAdsorbant)
                {
                    xA[co2] = xLo + i * uCX + offsetX;
                    yA[co2] = yLo + j * uCY + offsetY;
                    zA[co2] = 0 + k * uCZ + offsetZ;
                    co2++;
                }
            }
        }
    }
    cout << " no. of adsorbant molecules added = " << co2 << endl
         << "------------------------------------" << endl
         << endl;
    nAtomsTot += co2;

    cout << "nMethane mols = " << co << endl
         << "nAdsorbant mols = " << co2 << endl
    ...
    writer << "5   16.043000" << endl; //CH4 - adorbant
    writer << endl;
    writer << "Atoms" << endl;
    writer << endl;
    ...
    for (i = 0; i < nAdsorbant; i++)
    {
        typ = 5;
        molId++;
        mol++;
        writer << mol << '\t' << molId << '\t' << typ << '\t' << 0.0 << '\t' << xA[i] << '\t' << yA[i] << '\t' << zA[i] << endl;
    }
