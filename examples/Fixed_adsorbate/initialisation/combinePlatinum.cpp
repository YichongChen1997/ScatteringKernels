...
int main()
{
    ...
    // Read in the gas molecules within the near-surface layer
    ifstream readMethane("Platinum_15A_5.0.xyz"); // methane
    readMethane >> nWallMethane;

    getline(readMethane, line);
    getline(readMethane, line);
    ...
    // Fine the preferred timestep and add the position information
    for (i = 0; i < nWallMethane; i++)
    {
        readMethane >> typeName >> x >> y >> z;

        if (typeName == "1") // there are serval timestep to pick
        {
            typ = 5; // type 5 will be the methane molecules frozen within the near-surface layer
    ...
    int nPts0 = xM0.size();
    cout << "No. of methane within the near-surface layer: " << nPts0 << endl;
    cout << "--------------------------------------------" << endl;
    ...
    cout << "number of methane atoms to insert = " << nMethane << " + " << nPts0 << endl;
    ...
    double uCZ = (Lz - offsetZ - 2* rCut) / double(nPz);
    ...
    cout << " no. of methane molecules added = " << co << " + " << nPts0 << endl;
    nAtomsTot += co;

    cout << "nMethane mols = " << co + nPts0 << endl
    ...
    writer << "5   16.043000" << endl; //adsorbed CH4 - methane
    writer << endl;
    writer << "Atoms" << endl;
    writer << endl;
    ...
    for (i = 0; i < nPts0; i++)
    {
        ...
        writer << mol << '\t' << molId << '\t' << typ << '\t' << 0.0 << '\t' << xM0[i] << '\t' << yM0[i] << '\t' << zM0[i] << endl;
    }
